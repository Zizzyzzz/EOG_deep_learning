...
b4 = nn.Sequential(Inception1(448, 96, (192, 384), (192, 384, 576),48),
                   #nn.MaxPool2d(kernel_size=2, padding=(1, 0), stride=2))
...
class googlenet_121(nn.Module):  #w8 2 run
    def __init__(self, args): #**kwage是将除了前面显示列出的参数外的其他参数, 以dict结构进行接收.
        super(googlenet_121, self).__init__()
        self.model = nn.Sequential(b1, b2, b3, b4, nn.Linear(1104, args.class_num))
    def forward(self, x):
        temp = self.model(x)
        return temp


# net = googlenet_121(args)
# # # #
# # # #
# # # #
# # # #
