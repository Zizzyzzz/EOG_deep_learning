...
def test_c_output(iterator, model,save_path):
    print("begin to test")
    model.eval()
    test_loss = []
    y_true_list = []
    ...
    for idx, batch in enumerate(iterator):
        y_true = (batch[1].to(device))
        y_out = model(batch[0].type(torch.float32).to(device))
        feature_output = model.featuremap
        feature_output = torch.flatten(feature_output,start_dim=1)
        # y_prob = F.softmax(y_out, dim=-1)
        y_true_list.append(y_true.detach().cpu().numpy())
        last_layer_feature.append(feature_output.detach().cpu().numpy())
        # y_prob_list.append(y_prob.detach().cpu().numpy())
        # loss = loss_criterion(y_out, y_true.long())
        # test_loss.append(loss.item())
    # test_loss = np.hstack(test_loss).mean()
    y_true = np.hstack(y_true_list)
    last_layer_output = np.vstack(last_layer_feature)
    #
    # y_pred = np.argmax(y_prob, axis=-1)
    # Metric_count(y_true, y_pred)
    # c = np.where(y_true == y_pred)[0]
    ...
    # # np.savez(os.path.join(save_path, "resut.npz"), y_pred=y_pred, y_true=y_true, y_prob=y_prob)
    # print("test end")
    return y_true, last_layer_output
...
if __name__ == '__main__':
    ...
    for i_dataset in range(5):
        ...
        for test_sub_id in range(dataset_num[i_dataset]):
            ...
            acc, Sen, Spe, Blink_acc, Down_acc, Left_acc, Right_acc, Up_acc = test_c(test_data_loader, model_best, opts.output_path)
            y_true, last_layer_output = test_c_output(test_data_loader, model_best,opts.output_path)

            y_save_path = os.path.join('F:\ZengZhengPhd\zengz\phd1\分数据集的跑的EOG_SCnet\Scnet_model_validation\\tsne_model_feature\SCnet_afterCAM', str(i_dataset) + "AfterCAM_y_true"+str(test_sub_id))
            feature_save_path = os.path.join('F:\ZengZhengPhd\zengz\phd1\分数据集的跑的EOG_SCnet\Scnet_model_validation\\tsne_model_feature\SCnet_afterCAM', str(i_dataset) + "AfterCAM_feature_true"+str(test_sub_id))

            np.save(y_save_path, y_true)
            np.save(feature_save_path, last_layer_output)
        ...
        np.save(str(i_dataset) + "tsne_branch_AfterCAM_best_epoch200", best_epoch)
