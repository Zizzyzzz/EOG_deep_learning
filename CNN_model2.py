...
class Multi_scale_CNN(nn.Module):
    ...
    def forward(self, x):
        ...
        self.featuremap = fusion_featrue.detach()
...
class Multi_fusion_CNN(nn.Module):
    def __init__(self, args):
        super(Multi_fusion_CNN, self).__init__()
        self.CNN_branch_1 = nn.Sequential(
            nn.Conv1d(2, 32, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm1d(32),
            nn.LeakyReLU(.2),
            nn.Dropout(args.drop_out_prob),
            nn.MaxPool1d(kernel_size=3, stride=2))
        self.CNN_branch_1_1 = nn.Sequential(
            nn.Conv1d(32, 64, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm1d(64),
            nn.LeakyReLU(.2),
            nn.Dropout(args.drop_out_prob),
        ...
        self.CNN_branch_2 = nn.Sequential(
            ...
            nn.Dropout(args.drop_out_prob),
            nn.MaxPool1d(kernel_size=3, stride=2))
        self.CNN_branch_2_2 = nn.Sequential(
            nn.Conv1d(32, 64, kernel_size=5, stride=1, padding=1),
            nn.BatchNorm1d(64),
            nn.LeakyReLU(.2),
            nn.Dropout(args.drop_out_prob),
        ...
        self.CNN_branch_3 = nn.Sequential(
            ...
            nn.Dropout(args.drop_out_prob),
            nn.MaxPool1d(kernel_size=3, stride=2))
        self.CNN_branch_3_3 = nn.Sequential(
            nn.Conv1d(32, 64, kernel_size=7, stride=1, padding=1),
            nn.BatchNorm1d(64),
            nn.LeakyReLU(.2),
            nn.Dropout(args.drop_out_prob),
        ...
        self.early_fusion_net = nn.Sequential(
            nn.Conv1d(32, 16, kernel_size=9, stride=1, padding=1),
            nn.BatchNorm1d(16),
            nn.LeakyReLU(.2),
            nn.Dropout(args.drop_out_prob),
            nn.MaxPool1d(kernel_size=3, stride=2),
        ...
        self.later_fusion_net=nn.Sequential(
            nn.Conv1d(128, 16, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm1d(16),
            nn.LeakyReLU(.2),
            nn.Dropout(args.drop_out_prob),
            nn.MaxPool1d(kernel_size=3, stride=2)
    ...
    def forward(self, x):
        early_x_1 = self.CNN_branch_1(x)
        early_x_2 = self.CNN_branch_2(x)
        early_x_3 = self.CNN_branch_3(x)
        #
        # later_x_1 = self.CNN_branch_1_1(early_x_1)
        # later_x_2 = self.CNN_branch_2_2(early_x_2)
        # later_x_3 = self.CNN_branch_3_3(early_x_3)

        # later_x_1 = torch.flatten(later_x_1, start_dim=-2, end_dim=-1)
        # later_x_2 = torch.flatten(later_x_2, start_dim=-2, end_dim=-1)
        # later_x_3 = torch.flatten(later_x_3, start_dim=-2, end_dim=-1)
        # early_fusion_feature = torch.cat((early_x_1, early_x_2, early_x_3), dim=2)
        early_fusion_feature = torch.cat((early_x_1, early_x_2, early_x_3), dim=2)
        # fusion_feature = self.early_fusion_net(early_fusion_feature)
        # fusion_feature = torch.flatten(fusion_feature, start_dim=-2, end_dim=-1)
        # later_fusion_feature = torch.cat((later_x_1, later_x_2, later_x_3), dim=2)
        # later_fusion_feature = self.later_fusion_net(later_fusion_feature)
        # later_fusion_feature = torch.flatten(later_fusion_feature, start_dim=-2, end_dim=-1)
        # fusion_feature = torch.cat((fusion_feature, later_fusion_feature), dim=1)
        early_fusion_feature = torch.flatten(early_fusion_feature, start_dim=-2, end_dim=-1)
        fusion_feature = self.fc(early_fusion_feature)
        torch.cuda.empty_cache()
...
#         return x
class VGG_NC(nn.Module):
    def __init__(self, args):
        super(VGG_NC, self).__init__()
        self.model = nn.Sequential(
            nn.Conv1d(2, 900, kernel_size=3, stride=1, padding=1),
            nn.LeakyReLU(.2),
            nn.BatchNorm1d(900),

            nn.Conv1d(900, 512, kernel_size=3, stride=1, padding=1),
            nn.LeakyReLU(.2),
            nn.BatchNorm1d(512),
...
class CNN_eyesay(nn.Module):
    def __init__(self, args):
        super(CNN_eyesay, self).__init__()
        self.model = nn.Sequential(
...
def extract_data_set(i, i_dataset):
    dir ="D:\zengz\phd1\分数据集跑的EOGdeep\EOG_deep_learning_tiaocan\preprocessing"
    if i_dataset == 0:
